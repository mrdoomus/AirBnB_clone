...
class TestUserDoc(unittest.TestCase):
    '''
    calss to check documentation for User class
    '''

    @classmethod
    def setUpClass(cls):
        ''' This method run before all test and
            find into User class all functions.
            it store in a list of tuples
        '''
        cls.list_base_functions = inspect.getmembers(
            User, inspect.isfunction)

    def test_pep8_user_class(self):
        ''' Test that models/user.py conforms to PEP8 '''
        style = pep8.StyleGuide(quiet=True)
        result = style.check_files(['models/user.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found pep8 erros and warnings in user.py")

    def test_pep8_user_test(self):
        ''' Test that test/test_models/test_user.py conforms to PEP8 '''
        pep8style = pep8.StyleGuide(quiet=True)
        result = pep8style.check_files(
            ['tests/test_models/test_user.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found pep8 erros and warnings in test_user.py")
    ...
    def test_module_documentation(self):
        ...
        self.assertEqual(
            True, doc, mesg.format(user.__name__))

    def test_class_documentation(self):
        ''' check class User documentation '''
        doc = True
        try:
            len(User.__doc__) >= 1
        except Exception:
...
class TestUser(unittest.TestCase):
    ...
    def test_create_object_kwargs(self):
        base = User(name='name', school='Holberton')
        self.assertTrue(base.name)
    ...
    def test_create_object_simple_storage(self):
        ...
        self.assertEqual(base, dict_obects['User.' + base.id])
    ...
    def test_save_test_content(self):
        ...
        self.assertDictEqual(
            base.to_dict(), dict_to_load['User.' + base.id])
