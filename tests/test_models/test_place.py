...
class TestPlaceDoc(unittest.TestCase):
    '''
    calss to check documentation for Place class
    '''

    @classmethod
    def setUpClass(cls):
        ''' This method run before all test and
            find into Place class all functions.
            it store in a list of tuples
        '''
        cls.list_base_functions = inspect.getmembers(
            Place, inspect.isfunction)

    def test_pep8_place_class(self):
        ''' Test that models/place.py conforms to PEP8 '''
        style = pep8.StyleGuide(quiet=True)
        result = style.check_files(['models/place.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found pep8 erros and warnings in place.py")

    def test_pep8_place_test(self):
        ''' Test that test/test_models/test_place.py conforms to PEP8 '''
        pep8style = pep8.StyleGuide(quiet=True)
        result = pep8style.check_files(
            ['tests/test_models/test_place.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found pep8 erros and warnings in test_place.py")
    ...
    def test_module_documentation(self):
        ...
        self.assertEqual(
            True, doc, mesg.format(place.__name__))

    def test_class_documentation(self):
        ''' check class Place documentation '''
        doc = True
        try:
            len(Place.__doc__) >= 1
        except Exception:
...
class TestPlace(unittest.TestCase):
    ...
    def test_create_object_kwargs(self):
        base = Place(name='name', school='Holberton')
        self.assertTrue(base.name)
    ...
    def test_create_object_simple_storage(self):
        ...
        self.assertEqual(base, dict_obects['Place.' + base.id])
    ...
    def test_save_test_content(self):
        ...
        self.assertDictEqual(
            base.to_dict(), dict_to_load['Place.' + base.id])
